...
class LatinIME : InputMethodService(),
    ...
    private var chirpVoiceController: ChirpVoiceController? = null
    ...
    override fun onCreate() {
        ...
        if (!isUserLocked(this)) {
            chirpVoiceController = ChirpVoiceController(this).apply {
                setStateListener(object : ChirpVoiceController.StateListener {
                    override fun onStateChanged(state: ChirpVoiceController.State) {
                        suggestionStripView?.setVoiceKeyRecording(state == ChirpVoiceController.State.RECORDING)
                    }
                })
            }
        }
    ...
    override fun onDestroy() {
        ...
        chirpVoiceController?.destroy(); chirpVoiceController = null
    ...
    fun onEvent(event: Event) {
        ...
        if (event.keyCode == KeyCode.VOICE_INPUT) {
            val chirp = chirpVoiceController
            if (chirp != null && chirp.isEnabled()) {
                chirp.toggleRecording()
                return
            }
